...
create table if not exists member_receiving_account (
    id bigint auto_increment primary key,
    member_id varchar(64) not null,
...
create table if not exists async_compensation_task (
    ...
    key idx_task_status_updated_ts (task_status, updated_ts),
    key idx_lease_expire_ts (lease_expire_ts)
);
...
create table if not exists async_compensation_attempt (
    ...
    key idx_attempt_partition_started_ts (partition_no, started_ts)
);
